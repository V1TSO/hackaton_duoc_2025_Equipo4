...
async def handle_chat_message(
    ...
    session = get_or_create_session(user_id, data.session_id)
    if "error" in session:
    ...
    save_chat_message(session_id_str, "user", data.content)

    # 3. Cargar historial de chat (para el LLM)
    history = get_messages_by_session(session_id_str)
    ...
    assistant_message = save_chat_message(session_id_str, "assistant", response_text)
    ...
    if prediction_made and assessment_result:
        ...
        saved_assessment = save_assessment(user_id, assessment_result)
        
        if "id" in saved_assessment:
            # Vincular el assessment a la sesión de chat
            link_assessment_to_session(session_id_str, str(saved_assessment["id"]))
    ...
    final_history = get_messages_by_session(session_id_str)
